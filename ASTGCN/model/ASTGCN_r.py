...
class ASTGCN_submodule(nn.Module):
    ...
    def __init__(self, DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, cheb_polynomials, num_for_predict, len_input, num_of_vertices):
        ...
        self.final_conv = nn.Conv2d(int(len_input/time_strides), 2 * num_for_predict, kernel_size=(1, nb_time_filter))
    ...
    def forward(self, x):
        ...
        output = self.final_conv(x.permute(0, 3, 1, 2)).reshape(-1, self.num_for_predict, self.num_of_vertices, 2).permute(0, 2, 3, 1)
        # (b,N,F,T)->(b,T,N,F)-conv<1,F>->(b,c_out*T*2,N,1)->(b,c_out*T,N,2)->(b,N,T,2)
