...
def load_data(dconf):
    ...
    filename = os.path.join(datapath, dconf.dataset)
    ...
    if dconf.len_closeness > 0:
        ...
        for i in range(dconf.len_closeness - 1):
            X_closeness = np.concatenate(
                (X_closeness,
                 all_data[number_of_skip_hours - dconf.T_closeness * (2 + i):len_total - dconf.T_closeness * (2 + i)]),
                axis=1)
    if dconf.len_period > 0:
        ...
        for i in range(dconf.len_period - 1):
            X_period = np.concatenate(
                (X_period,
                 all_data[number_of_skip_hours - dconf.T_period * (2 + i):len_total - dconf.T_period * (2 + i)]),
                axis=1)
    if dconf.len_trend > 0:
        ...
        for i in range(dconf.len_trend - 1):
            X_trend = np.concatenate(
                (X_trend, all_data[number_of_skip_hours - dconf.T_trend * (2 + i):len_total - dconf.T_trend * (2 + i)]),
                axis=1)
    ...
    X_train = X_train.reshape(X_train.shape[0], -1, 2, X_train.shape[-1])
    X_train = np.transpose(X_train, (0, 3, 1, 2))
    Y_train = Y_train.reshape(Y_train.shape[0], Y_train.shape[1], -1)
    Y_train = np.expand_dims(np.transpose(Y_train, (0, 2, 1)), 2)

    X_val = X_val.reshape(X_val.shape[0], X_val.shape[1], -1)
    X_val = X_val.reshape(X_val.shape[0], -1, 2, X_val.shape[-1])
    X_val = np.transpose(X_val, (0, 3, 1, 2))
    Y_val = Y_val.reshape(Y_val.shape[0], Y_val.shape[1], -1)
    Y_val = np.expand_dims(np.transpose(Y_val, (0, 2, 1)), 2)

    X_test = X_test.reshape(X_test.shape[0], X_test.shape[1], -1)
    X_test = X_test.reshape(X_test.shape[0], -1, 2, X_test.shape[-1])
    X_test = np.transpose(X_test, (0, 3, 1, 2))
    Y_test = Y_test.reshape(Y_test.shape[0], Y_test.shape[1], -1)
    Y_test = np.expand_dims(np.transpose(Y_test, (0, 2, 1)), 2)

    print('len_train=' + str(X_train.shape[0]))
    print('len_val=' + str(X_val.shape[0]))
    print('len_test =' + str(X_test.shape[0]))
