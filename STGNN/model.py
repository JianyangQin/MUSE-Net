...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, device, outfea, max_len=12):
...
class SGNN(nn.Module):
    def __init__(self, device, outfea):
    ...
    def forward(self, x):
        ...
        R = torch.relu(torch.softmax(a, -1)) + torch.eye(x.shape[1]).to(self.device)
...
class STGNNwithGRU(nn.Module):
    def __init__(self, device, outfea):
        ...
        self.device =device
        self.sgnnh = nn.ModuleList([SGNN(device, outfea) for i in range(12)])
        self.sgnnx = nn.ModuleList([SGNN(device, outfea) for i in range(12)])
        self.gru = nn.ModuleList([GRU(outfea) for i in range(12)])

    def forward(self, x):
        B, T, N, F = x.shape
        hidden_state = torch.zeros([B, N, F]).to(self.device)
        output = []
...
class STGNN(nn.Module):
    def __init__(self, device, infea, outfea, L, d):
        super(STGNN, self).__init__()

        self.start_emb = nn.Linear(infea, outfea)
        self.end_emb = nn.Linear(outfea, infea)

        self.stgnnwithgru = nn.ModuleList([STGNNwithGRU(device, outfea) for i in range(L)])
        self.positional_encoding = PositionalEncoding(device, outfea)
