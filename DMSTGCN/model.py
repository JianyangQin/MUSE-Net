...
class DMSTGCN(nn.Module):
    def __init__(self, device, num_nodes, dropout=0.3, in_len=11,
        ...
        for b in range(blocks):
            ...
            for i in range(layers):
                ...
                if normalization == "batch":
                    ...
                    self.normal.append(nn.LayerNorm([residual_channels, num_nodes, in_len + 1 - receptive_field - new_dilation + 1]))
                new_dilation *= 2
        ...
        self.end_conv_1 = nn.Conv2d(in_channels=int(skip_channels * (12 + 10 + 9 + 7 + 6 + 4 + 3 + 1) / in_dim),
    ...
    def forward(self, inputs, ind):
        ...
        for i in range(self.blocks * self.layers):
            ...
            if isinstance(skip, int):  # B F N T
                skip = s.transpose(2, 3).reshape([s.shape[0], -1, s.shape[2], self.in_dim]).contiguous()
            else:
                skip = torch.cat([s.transpose(2, 3).reshape([s.shape[0], -1, s.shape[2], self.in_dim]), skip], dim=1).contiguous()
...
class DMSTGCN_2flow(nn.Module):
    def __init__(self, device, num_nodes, dropout=0.3, in_len=11,
                 out_dim=12, residual_channels=16, dilation_channels=16, end_channels=512,
                 kernel_size=2, blocks=4, layers=2, days=288, dims=40, order=2, in_dim=9, normalization="batch"):
        super(DMSTGCN_2flow, self).__init__()
        self.outflow_encoder = DMSTGCN(device=device, num_nodes=num_nodes, dropout=dropout, in_len=in_len, out_dim=out_dim,
                                       ...
                                       days=days, dims=dims, order=order, in_dim=in_dim // 2, normalization=normalization)
        self.inflow_encoder = DMSTGCN(device=device, num_nodes=num_nodes, dropout=dropout, in_len=in_len, out_dim=out_dim,
                                      residual_channels=residual_channels, dilation_channels=dilation_channels,
                                      end_channels=end_channels, kernel_size=kernel_size, blocks=blocks, layers=layers,
                                      days=days, dims=dims, order=order, in_dim=in_dim // 2, normalization=normalization)

    def forward(self, input, ind):
        outflow = self.outflow_encoder(input[:, 0:1, :, :], ind)
        inflow = self.inflow_encoder(input[:, 1:2, :, :], ind)
        y = torch.cat([outflow, inflow], dim=-1)
        return y
